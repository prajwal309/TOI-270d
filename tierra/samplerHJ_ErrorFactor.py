...
def logLikelihood(theta, Wavelength, WavelengthLower, WavelengthUpper, Spectrum, SpectrumErr):
    ...
    log_f, P0, T0, ALR, TInf, MR_N2Log, MR_COLog, MR_H2OLog, \
    ...
    global BiggestChiSqr, ParameterNames, CurrentSaveName, CurrentzStep
    ...
    f = 10**log_f
    SigmaErr = SpectrumErr*SpectrumErr+f*f*BinnedModel*BinnedModel

    Residual = np.sum((Spectrum-BinnedModel)**2/(SigmaErr))
    ChiSqr = -(0.5*Residual + np.sum(np.log(SigmaErr)))


    if ChiSqr>BiggestChiSqr:
       print("Saving the best model.")
       BiggestChiSqr = ChiSqr
...
def RunMCMC( PlanetParamsDict, StellarParamsDict, CSLocation=None, AssignedzStep=0.25, SubFolderName="CS_1", SaveName="Default", NSteps=1500, NCORES=4):
    ...
    global zStep, BiggestChiSqr, CurrentSaveName, CurrentzStep
    CurrentSaveName = SaveName
    BiggestChiSqr = -np.inf
    ...
    f_Init = np.random.normal(1.1,0.1, nWalkers)                               #Pressure at R_p in atm
    ...
    ParameterNames = ["f", "P0", "T0", "ALR", "TInf", "MR_N2Log", "MR_COLog", \
    "MR_H2OLog", "MR_CO2Log",  "MR_CH4Log", "MR_O3Log"]

    StartingGuess = np.column_stack((f_Init, P0Init, T0Init, ALRInit, TInfInit, \
    ...
    Best_f, BestP0, BestT0, BestALR, BestTInf, BestMR_N2Log, BestMR_COLog, BestMR_H2OLog, \
    ...
    #Bin the model to maek it better
