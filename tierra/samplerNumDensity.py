...
def logLikelihood(theta, Wavelength, WavelengthLower, WavelengthUpper, Spectrum, SpectrumErr):
    ...
    if max([MR_N2, MR_CO, MR_H2O, MR_CO2, MR_O3, MR_CH4])>0.5:
        return -np.inf
...
def RunMCMC( PlanetParamsDict, StellarParamsDict, CSLocation=None, AssignedzStep=0.25, SubFolderName="CS_1", SaveName="Default", NSteps=1500, NCORES=4):
    ...
    global LeastResidual, CurrentSaveName, CurrentzStep
    ...
    if "HJ" in CurrentSaveName:
        Wavelength, WavelengthLower, WavelengthUpper, Spectrum, SpectrumErr  = np.loadtxt("FittingData/HotJupiter_FIT.data", delimiter=",", unpack=True)
    elif "SE" in CurrentSaveName:
        Wavelength, WavelengthLower, WavelengthUpper, Spectrum, SpectrumErr  = np.loadtxt("FittingData/SuperEarth_FIT.data", delimiter=",", unpack=True)
    else:
    ...
    N0_N2LogInit = np.random.normal(N0_N2Log, 0.1, nWalkers)                              #Mixing ratio for nitrogen
    N0_COLogInit = np.random.normal(N0_COLog, 0.1, nWalkers)                              #Mixing ratio for carbonmonoxide
    N0_H2OLogInit = np.random.normal(N0_H2OLog, 0.1, nWalkers)                            #Mixing ratio for water
    N0_CO2LogInit = np.random.normal(N0_CO2Log, 0.1, nWalkers)                            #Mixing ratio for carbondioxide
    N0_CH4LogInit = np.random.normal(N0_CH4Log, 0.1, nWalkers)                            #Mixing ratio for methane
    N0_O3LogInit = np.random.normal(N0_O3Log, 0.1, nWalkers)                              #Mixing ratio for oxygen
    N0_H2LogInit = np.random.normal(N0_H2Log, 0.1, nWalkers)                              #Mixing ratio for oxygen
    ...
    Total_N0 = N0_H2+N0_N2+N0_CO+N0_H2O+N0_CO2+N0_O3+N0_CH4
